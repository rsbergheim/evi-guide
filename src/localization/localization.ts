export default interface Localization {
    aksepterBrukervilkaar: string;
    hjem: string;
    turer: string;
    kurs: string;
    utstyr: string;
    info: string;
    kontaktinfo: string;
    vilkår: string;
    ja: string;
    nei: string;
    epost: string;
    lesMer: string;
    coursesHeading: string; // Title for the courses page
    beginnerLevelTitle: string; // Title for beginner-level courses
    beginnerLevelDescription: string; // Description for beginner-level courses
    expertLevelTitle: string; // Title for expert-level courses
    expertLevelDescription: string; // Description for expert-level courses
    guidedToursTitle: string; // Title for guided tours
    guidedToursDescription: string; // Description for guided tours
    bestill: string;
    pålDescription: string;
    kontaktinfoHeader: string;
    kontaktinfoDescription1: string;
    kontaktinfoDescription2: string;
    kontaktinfoDescription3: string;
    vilkårHeading: string;
    vilkårPoint1: string;
    vilkårPoint2: string;
    vilkårPoint3: string;
    eviskiDescription1: string;
    eviskiDescription2: string;
    eviskiDescription3: string;
    antallPersoner: string;
    dato: string;
    område: string;
}
